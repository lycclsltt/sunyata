...
### 健康检查
基于Consul的Check机制，服务注册后，自动添加一个定期的检查任务。默认为TCP端口检查，支持TCP/HTTP RPC服务端，UDP服务端暂不支持。一旦服务进程挂掉或服务发生异常，那么客户端会请求到其他健康的服务端节点上。由于健康检查存在时间间隔，可能服务端进程挂掉后，仍需等待一段时间才被Consul发现，这时客户端如果请求到挂掉的服务节点上失败后，客户端会尝试请求其他服务节点进行重试。
...
2.客户端在发起请求时做轮询
> 当多个服务端节点在注册中心注册后（当前为consul），客户端发起请求时会轮询发现到的服务端节点。按照轮询策略请求当前的服务端节点时，如果失败，客户端会自动重试，尝试请求下一个，直到请求次数满三次均失败为止（仅当请求到异常的服务端节点时，才会触发自动重试）。
